...
The [public authoring contract](v0.1.0a3-public-authoring-contract.md) is now a
prerequisite for `CRB-08`. Its API and protocol changes invalidate the current
candidate artifact identities for publication. The existing local and Mantra
receipts remain evidence for commit `0ee1764`; `CRB-05`, `CRB-06`, and `CRB-07`
must run again against the newly built wheel before `CRB-08` can close.
...
The gap descriptions below record the source state inspected when this
contract was written. Current implementation and acceptance results are
tracked in the adjacent checklist and [release report](../releases/0.1.0a3.md).
Use the [public API reference](../reference/api.md) for current interfaces.
...
9. Install the same candidate wheel on the Mantra GPU host and run the agreed
   ...
   VIPER import source.
10. Complete the public authoring contract, rebuild the distributions, and
    repeat the local, clean-install, CI, and Mantra gates against the new wheel.
11. Update `CHANGELOG.md`, add the `0.1.0a3` release evidence, and prepare the
    merge and signed-tag inputs. Merge, tag, and publish only after the user
...
| Retained behavior gate | `CRB-04` retains the Phase 4+ execution and protocol work. The functional suite passed 331 tests with 6 expected skips and 2 version-identity tests deferred to `CRB-05`; its sole generated-project benchmark failure exposed two stale source-commit lookups for a plan-owned benchmark specification. After both lookup sites were repaired, the complete generated-project acceptance test passed. Ruff formatting and lint checks pass, and Pyright reports 0 errors. |
| Distribution inventories | `CRB-05` built `viper_provenance-0.1.0a3-py3-none-any.whl` (220336 bytes, SHA-256 `c81f7f13b870e6533cfabdb7038efb08814aa61abe289824e2ae8d17798b9f79`) and `viper_provenance-0.1.0a3.tar.gz` (326556 bytes, SHA-256 `4626d0ffab0bb563dd59b6fe84f54db24b8cffc7a8211560e3671daa6916ec3f`). Archive inspection found every retained knowledge module, rejected every forbidden path, and found neither removed public operation in the packaged API, CLI, or MCP modules. Twine accepted both archives. |
| Local gate output | `CRB-06` installed the recorded wheel in a clean Python 3.14.5 environment outside the checkout. All 13 installed public-API tests, four CLI checks, the CPU quickstart, and the complete generated-project acceptance passed; the environment reported `viper-provenance 0.1.0a3`. The repository gates then passed with 256 unit/contract tests, 75 integration tests, 334 host-independent release tests, Ruff, Pyright, and the CPU quickstart. The Python 3.11-3.14 CI matrix remains the final `CRB-06` gate. |
| Mantra receipt | `CRB-07` passed on 2026-09-07. An isolated Python 3.13 environment installed `viper_provenance-0.1.0a3-py3-none-any.whl` with SHA-256 `c81f7f13b870e6533cfabdb7038efb08814aa61abe289824e2ae8d17798b9f79`. VIPER observed one NVIDIA L4, driver `580.126.20`, CUDA `12.9`, and 23,659,151,360 bytes of device memory. In the same process, Mantra commit `6916d01404ecebe4ee21b29ca74724a8a8ced7c2` ran `mantra.utils.ridge_gpu.ridge_fit_predict_torch` on CUDA. Its finite `(64, 3)` prediction differed from Mantra's NumPy reference by at most `1.52587890625e-05`, below the `2.5e-4` absolute and relative tolerances. The Mantra worktree remained clean. The machine-readable receipt is [`receipts/v0.1.0a3-mantra-gpu.json`](receipts/v0.1.0a3-mantra-gpu.json), whose SHA-256 is `79f4850154d9fffb80485bb67fb9b6296b06d868f7083e632fa1d30eb8675b99`. |
...
- [x] `P0-CRB-04`: Run local and CI release gates against version `0.1.0a3` and
