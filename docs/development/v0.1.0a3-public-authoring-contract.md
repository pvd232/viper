...
`0ee17641f58819dae17fb5fb2db891cc11460ce8`.
...
and project vocabulary with one lifecycle-based API and adds diagnostic stages.
It does not authorize merging, tagging, TestPyPI publication, or PyPI
publication.
...
| `PAC-07` | Migrate the public API, CLI, generated workspace, documentation, schemas, and examples to the approved vocabulary. |
...
outputs, and leaves verified artifacts after it runs.
...
[`viper.params`](../../src/viper/params.py) defines `ParameterSet` and
...
flowchart LR
    ...
    class Freeze,Dispatch,Run,Records implementation
    class Eval gap
...
flowchart LR
    ...
    Dispatch --> Run["Run stage"]
    Run --> Verify["Verify declared bytes"]
    Verify --> Artifacts["Recorded artifacts"]

    class Input,Config,Outputs proposed
    class Stage,Freeze,Dispatch,Run,Verify,Artifacts implementation
...
flowchart TD
    ...
    Download --> Artifacts["Verified artifacts"]
    Build --> Artifacts
    Embed --> Artifacts
    Train --> Artifacts
    Eval --> Artifacts
    Diagnostic --> Reports["Terminal diagnostic artifacts"]
    Artifacts --> Catalog["Catalog + reuse"]
    Reports --> Catalog

    class Workspace,Package current
    class Ref,Plan,Kinds proposed
    class Download,Build,Embed,Train,Eval,Diagnostic implementation
    class Artifacts,Reports,Catalog evidence
    classDef current fill:#1e3a8a,stroke:#60a5fa,color:#ffffff,stroke-width:2px
    classDef proposed fill:#581c87,stroke:#d8b4fe,color:#ffffff,stroke-width:2px
    classDef implementation fill:#115e59,stroke:#5eead4,color:#ffffff,stroke-width:2px
...
The public training shape is:

```python
class RidgeTrainConfig(viper.config.TrainConfig):
    learning_rate: float
    epochs: int


training_outputs = viper.outputs.TrainOutputs(
    parameters=viper.output(
        path="parameters.json",
        loader=load_parameters,
        data_role="training",
    ),
    resume_state=viper.output(
...
@viper.stages.train(config=RidgeTrainConfig, outputs=training_outputs)
def fit(context: viper.Context[RidgeTrainConfig, viper.outputs.TrainOutputs]):
...
4. Generate each run location as
   ...
   products.
5. Generate reusable pointer locations from run, stage, and output identity.
   Use
...
6. Make `build` accept every valid workspace-defined output name. Enforce
   `parameters` and `resume_state` only in `TrainOutputs`, and `predictions`
   only in `EvalOutputs`.
7. Add `diagnostic` to the authoring decorator, draft and frozen unions,
   resolved unions, config variants, execution, reuse, schema generation,
   capabilities, and verification.
8. Require diagnostic stages to consume only earlier stage outputs, allow
...
9. Replace every stage-kind fallback with an exhaustive branch whose default
   raises an unsupported-kind error naming the received discriminator.
10. Replace VIPER-owned `project` symbols and prose with `workspace` for
...
11. Update generated workspaces, docs, examples, schemas, CLI and MCP
    operations, tests, and release checks. Do not add compatibility aliases for
    the retired alpha API.
12. Build one new candidate, validate it outside the checkout, run that exact
    wheel through Mantra, and hold the signed tag until the user approves
    publication.
...
| Mantra receipt | Mantra commit, wheel SHA-256, environment, GPU identity, command, numerical result, tolerance, and clean-worktree status. |
...
|---|---|
| `authoring.config.only` | Public imports, schemas, generated code, and docs expose `config`; rejected fixtures prove that `params`, `parameter_model`, and `stage_params` are not accepted in affected version-2 documents. |
...
| Documentation | Migrate README, tutorials, how-to guides, API reference, explanations, internal formalism, examples, and release notes; explain the output-to-artifact transition once. |
...
and allows a later evaluation stage to consume the recorded parameters. A
terminal diagnostic stage consumes the evaluation artifact and records a plot
and a report without an objective.
...
7. Complete `PAC-07`, then run all local, distribution, clean-install, CI, and
   Mantra gates in `PAC-08`.
8. Prepare `PAC-09` and stop. Publication remains a separate user-approved
   action.
...
- [ ] `P0-PAC-01` / `PAC-01`: Migrate the config vocabulary across public,
  serialized, runtime, and verification surfaces. Gate:
...
- [ ] `P3-PAC-07` / `PAC-07`: Update public APIs, CLI, MCP, schemas,
  documentation, examples, and generated workspaces. Gate: public inventory,
  documentation checks, generated-workspace acceptance, and CPU quickstart.
  Commit: public surface and documentation only.
- [ ] `P3-PAC-08` / `PAC-08`: Run full repository checks, build fresh
  ...
  `authoring.mantra.accepted`. Commit: release evidence only.
...
| `P3-PAC-07` | `PAC-07` | `PAC-06` | Public inventory and documentation acceptance |
| `P3-PAC-08` | `PAC-08` | `PAC-07` | `authoring.clean.install`, `authoring.mantra.accepted` |
...
This contract uses four actions:
...
| `P3-PAC-08` | `add` | Test outputs, distribution inventories, clean-install receipt, CI results, release evidence, and Mantra receipt |
