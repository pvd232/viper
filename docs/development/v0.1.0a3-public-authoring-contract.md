...
`model`, `resume_state`, and `predictions`; they are not storage
categories.
...
state needed to resume training. [`viper.repository`](../../src/viper/repository.py)
...
| `OutputName` | Validated identifier used while a stage output is declared or referenced before completion. |
| `TrainOutputs[T]` | Requires `model: T` and `resume_state: T`. |
...
def fit(context: Context[RidgeTrainConfig, TrainOutputs]):
    context.outputs.model.write_bytes(...)
```
...
10. Make `build` accept every valid workspace-defined output name. Enforce
   `model` and `resume_state` only in `TrainOutputs`, and `predictions`
   only in `EvalOutputs`.
...
| `authoring.output.required` | Missing `model` or `resume_state` rejects a training declaration; missing `predictions` rejects an evaluation declaration. |
...
| Output authoring | Add `viper.outputs`, `StageOutputs`, `TrainOutputs`, and `EvalOutputs`; rename authoring fields and context access from artifacts to outputs. |
...
| Built-in names | Retain training `model`, replace `state` with `resume_state`, and replace evaluation `preds` with `predictions`. |
...
`model` and `resume_state`, and decorates `fit` with `config=` and
...
and allows a later evaluation stage to consume the recorded model. The
