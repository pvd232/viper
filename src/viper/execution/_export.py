...
from .._config.validation import verify_config_type_bytes
...
from ..stages import DownloadSpec
...
def _verify_identity(raw: bytes, *, sha256: str, size: int, label: str) -> None:
    """Match one retained payload to its frozen byte identity."""
    if len(raw) != size or hashlib.sha256(raw).hexdigest() != sha256:
        raise RunExportError(f"{label} identity differs")


def _verify_plan_local_dependencies(
    plan: VerifiedRunPlan,
    fetcher: _RecordingFetcher | _BundleFetcher,
) -> None:
    """Verify host payloads named by every planned download stage."""
    for stage in plan.stages.values():
        if not isinstance(stage, DownloadSpec) or not isinstance(
            stage.http, WorkspaceHttpImplementationSpec
        ):
            continue
        http = stage.http
        implementation_raw = fetcher(
            GitFileRef(
                repository=plan.run.source.repository,
                commit=plan.run.source.commit,
                path=http.implementation.path,
            )
        )
        _verify_identity(
            implementation_raw,
            sha256=http.implementation.sha256,
            size=http.implementation.bytes,
            label=f"HTTP implementation {http.id!r}",
        )
        if http.config_type.owner == "workspace":
            config_raw = fetcher(
                GitFileRef(
                    repository=plan.run.source.repository,
                    commit=plan.run.source.commit,
                    path=http.config_type.path,
                )
            )
        else:
            config_raw = fetcher.read_viper_source(http.config_type)
        verify_config_type_bytes(http.config_type, config_raw)
        for executable in http.executables:
            raw = fetcher.read_external_executable_spec(executable)
            _verify_identity(
                raw,
                sha256=executable.sha256,
                size=executable.bytes,
                label=f"executable {executable.executable_id!r}",
            )
...
def export_run(
    ...
    *,
    ...
    _verify_plan_local_dependencies(verified.plan, recording)
...
def verify_run_bundle(
    ...
    *,
    ...
    try:
        ...
        _verify_plan_local_dependencies(verified.plan, fetcher)
